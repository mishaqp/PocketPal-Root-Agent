...
export function migrateContextInitParams(
  ...
  // Migration from 2.2 to 2.3: Android must opt in to an accelerator.
  // An implicit device choice with all layers offloaded can select OpenCL,
  // which only accepts a small subset of GGUF quantizations and may crash
  // while loading other models. Keep explicitly selected GPU/HTP backends.
  if (migratedParams.version === '2.2') {
    if (
      Platform.OS === 'android' &&
      (!Array.isArray(migratedParams.devices) || migratedParams.devices.length === 0)
    ) {
      migratedParams.devices = ['CPU'];
      migratedParams.n_gpu_layers = 0;
    }
...
export function createDefaultContextInitParams(): ContextInitParams {
  return {
    ...
    cache_type_k: 'f16',
    cache_type_v: 'f16',
    n_gpu_layers: Platform.OS === 'android' ? 0 : 99,
    use_mlock: false,
    use_mmap: Platform.OS === 'android' ? 'false' : 'true',

    // New v2.0 parameters
    // Android accelerators must be chosen explicitly. This avoids loading
    // unsupported quantizations through OpenCL by accident.
    devices: Platform.OS === 'android' ? ['CPU'] : undefined,
